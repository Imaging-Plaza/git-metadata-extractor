...
This guide explains the architecture of the `git_metadata_extractor/agents/llm/` sub-package and walks through the exact steps required to add a new LLM-backed agent. Use the existing repository/person/organization/article/membership/contribution agents as reference implementations.
...
```
git_metadata_extractor/agents/llm/
...
### `V2LLMRuntime` (`git_metadata_extractor/agents/llm/runtime.py`)
...
The strict model comes from `git_metadata_extractor/schema/models/strict.py` (TTL-shape-aligned).

### `RuntimeAgent` protocol (`git_metadata_extractor/agents/contracts.py`)
...
_PROMPTS_PACKAGE = "git_metadata_extractor.agents.llm.myagent.prompts"
...
if TYPE_CHECKING:
    from git_metadata_extractor.providers.base import MyProvider
...
```
git_metadata_extractor/agents/llm/organization/
├── __init__.py
├── agent.py
└── prompts/
...
```python
# git_metadata_extractor/agents/llm/organization/agent.py
from __future__ import annotations
...
from git_metadata_extractor.schema.models.strict import OrganizationModel          # strict model
from git_metadata_extractor.agents.llm.runtime import LLMRuntimeError, V2LLMRuntime

logger = logging.getLogger(__name__)

_PROMPTS_PACKAGE = "git_metadata_extractor.agents.llm.organization.prompts"
_SYSTEM_PROMPT = load_prompt(_PROMPTS_PACKAGE, "system_prompt.md")
_USER_PROMPT_TEMPLATE = load_prompt(_PROMPTS_PACKAGE, "user_prompt.md")
...
```python
# git_metadata_extractor/agents/llm/organization/__init__.py
from git_metadata_extractor.agents.llm.organization.agent import LLMOrganizationAgentV2

__all__ = ["LLMOrganizationAgentV2"]
```

### Step 6 — Export from `git_metadata_extractor/agents/llm/__init__.py`
...
### Step 7 — Export from `git_metadata_extractor/agents/__init__.py`

```python
from git_metadata_extractor.agents.llm import LLMOrganizationAgentV2, LLMPersonAgentV2, LLMRepositoryAgentV2
# … add to __all__ …
```

---

## Wiring into the registry (`git_metadata_extractor/agents/registry.py`)

`AgentRuntimeRegistry` routes LLM agents by `(runtime, stage_key)`. Add a stage constant and a routing branch:

```python
from git_metadata_extractor.agents.llm import LLMOrganizationAgentV2   # add import
...
## Wiring into the orchestrator (`git_metadata_extractor/pipeline/orchestrator.py`)

`PipelineOrchestrator` constructs `AgentRuntimeRegistry`. Add the new agent parameter:

```python
from git_metadata_extractor.agents import LLMOrganizationAgentV2   # add to existing import block
...
The rule-based agents (`PersonAgentV2`, `OrganizationAgentV2`, etc.) live in `git_metadata_extractor/agents/` (not inside `llm/`). They satisfy the same `RuntimeAgent` protocol and are the default when `agent_runtime=rule_based`. The orchestrator's `_rule_based_runners` dict maps stage keys to their `.run` methods.
...
The generated model classes land in `git_metadata_extractor/schema/models/agent.py` (permissive) and `git_metadata_extractor/schema/models/strict.py` (strict).

### `generate_uuid()`

Use `git_metadata_extractor.agents.models.generate_uuid()` (UUIDv4) for any stable person/entity UUID generated inside an agent. Do not use deterministic UUIDv5 emitters for agent outputs.
