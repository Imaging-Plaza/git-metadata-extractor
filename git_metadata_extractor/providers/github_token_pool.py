...
- `git_metadata_extractor/providers/github_provider.py` — REST calls in the v2
  pipeline, via `github_auth_headers()`.
- `git_metadata_extractor/providers/github_accounts/{users,orgs}_parser.py` — GraphQL + REST calls in the
  legacy parsers still invoked by the v2 hybrid runtime.

`git_metadata_extractor/app.py` startup splits a comma-separated `GME_GITHUB_TOKEN` into a
