...
from git_metadata_extractor.providers.infoscience_models import (
    InfoscienceAuthor,
    InfoscienceOrgUnit,
...
async def search_authors(
    ...
    person_response = await _make_api_request(
    ...
    pub_response = await _make_api_request(
        "/discover/search/objects",
        params=pub_params,
    )
...
async def search_labs(
    ...
    orgunit_response = await _make_api_request(
        "/discover/search/objects",
        params=orgunit_params,
    )
    ...
    logger.info(
        f"No organizational units found, searching publications for lab info: {name}",
    )
    params = {
        "query": name,
        "size": max_results,
...
async def search_infoscience_publications_tool(
    query: str,
    max_results: int = 10,
) -> str:
    """
    ...
    **CRITICAL: If this tool returns 0 results, STOP searching for this entity because the results were 0 - it is not in Infoscience. Do not try variations or search again.**
    ...
    """
    logger.info(
        f"🔍 Agent tool called: search_infoscience_publications_tool(query='{query}', max_results={max_results})",
    )
    ...
    try:
        ...
        logger.info(
            f"✓ Infoscience publications search returned {result.total_results} total results",
        )
        ...
        logger.error(
            f"✗ Error in search_infoscience_publications_tool: {e}",
            exc_info=True,
        )
...
async def search_infoscience_authors_tool(name: str, max_results: int = 10) -> str:
    ...
    **CRITICAL: If this tool returns 0 results, STOP searching for this entity because the results were 0 - it is not in Infoscience. Do not try variations or search again.**
    ...
    """
    logger.info(
        f"🔍 Agent tool called: search_infoscience_authors_tool(name='{name}', max_results={max_results})",
    )
    ...
    try:
        ...
        logger.info(
            f"✓ Infoscience authors search returned {result.total_results} total results",
        )
        markdown_result = result.to_markdown()
...
async def search_infoscience_labs_tool(name: str, max_results: int = 10) -> str:
    ...
    **CRITICAL: If this tool returns 0 results, STOP searching for this entity because the results were 0 - it is not in Infoscience. Do not try variations or search again.**
    ...
    """
    logger.info(
        f"🔍 Agent tool called: search_infoscience_labs_tool(name='{name}', max_results={max_results})",
    )
    ...
    try:
        ...
        logger.info(
            f"✓ Infoscience labs search returned {result.total_results} total results",
        )
        markdown_result = result.to_markdown()
...
async def get_author_publications_tool(author_name: str, max_results: int = 10) -> str:
    ...
    """
    logger.info(
        f"🔍 Agent tool called: get_author_publications_tool(author_name='{author_name}', max_results={max_results})",
    )
    ...
    try:
        ...
        if result.total_results > 0:
            logger.info(
                f"✓ Found {result.total_results} publications for author '{author_name}'",
            )
            markdown_result = (
