...
| **[`ontology-explorer.html`](ontology-explorer.html)** | **Open it in a browser** — the whole proposal as one self-contained page: classes with their properties, shapes, GME provenance and real platform attribution; the term network as a routed board or an organic graph; the alignment axioms; every enumeration; and a sortable table of all 282 terms. Generated from the TTLs by [`build_explorer.py`](build_explorer.py), no CDN, no server. |
