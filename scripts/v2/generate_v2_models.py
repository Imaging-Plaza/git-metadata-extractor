...
def _schema_hashes() -> dict[str, str]:
    return {
        schema_name: hashlib.sha256(
            (STRICT_SCHEMA_DIR / schema_name).read_bytes(),
        ).hexdigest()
        for schema_name in sorted(MODEL_SCHEMA_FILES.values())
    }


def _build_embedded_bundle(path: Path) -> None:
    bundle: dict[str, object] = {
        "$schema": "http://json-schema.org/draft-07/schema#",
        "title": "EntitiesBundle",
        "type": "object",
        "properties": {},
        "$defs": {},
    }
    ...
    for model_name, schema_name in MODEL_SCHEMA_FILES.items():
        schema_payload = json.loads((STRICT_SCHEMA_DIR / schema_name).read_text())
        schema_payload["title"] = model_name
        definitions[model_name] = schema_payload
        property_name = model_name.removesuffix("Model").lower()
        properties[property_name] = {"$ref": f"#/$defs/{model_name}"}

    path.write_text(json.dumps(bundle, indent=2) + "\n", encoding="utf-8")


def _datamodel_codegen_command(bundle_path: Path, output_path: Path) -> list[str]:
...
def _run_codegen(bundle_path: Path, output_path: Path) -> None:
    command = _datamodel_codegen_command(bundle_path, output_path)
    try:
...
def _render_expected_output() -> str:
    hashes = _schema_hashes()
    with tempfile.TemporaryDirectory(prefix="v2-model-codegen-") as tmp_dir:
        temp_dir = Path(tmp_dir)
        bundle_path = temp_dir / "entities_bundle.schema.json"
        raw_output_path = temp_dir / "entities.py"
        _build_embedded_bundle(bundle_path)
        _run_codegen(bundle_path, raw_output_path)
...
def _check_models_file() -> int:
    if not GENERATED_MODELS_PATH.exists():
        print(f"Missing generated models file: {GENERATED_MODELS_PATH}")
        print("Run: just v2-models-generate")
        return 1

    expected = _render_expected_output()
    actual = GENERATED_MODELS_PATH.read_text(encoding="utf-8")
    if actual == expected:
        print("Generated models are up-to-date.")
        return 0

    current_hashes = _schema_hashes()
    embedded_hashes = _extract_embedded_hashes(GENERATED_MODELS_PATH)
    changed_schemas = [
        schema_name
    ...
    if changed_schemas:
        ...
        print("Generated models are stale (codegen output drifted). Regenerate models.")
...
def _generate_models_file() -> int:
    expected = _render_expected_output()
    GENERATED_MODELS_PATH.parent.mkdir(parents=True, exist_ok=True)
    GENERATED_MODELS_PATH.write_text(expected, encoding="utf-8")
    print(f"Generated {GENERATED_MODELS_PATH.relative_to(REPO_ROOT)}")
    return 0
...
def main() -> int:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="Verify generated models are in sync with strict schemas.",
    )
