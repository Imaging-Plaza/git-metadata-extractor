...
- `schema:alternateName` is an **agent-only** intermediate field for organization entities: it is set by the organization agent, consumed by membership/article/reconciliation stages for alias matching, and stripped by reconciliation before final output and strict validation. It must remain in the agent schema (`src/v2/schemas/agent/organization.schema.json`) but must not appear in the strict schema (`src/v2/schemas/strict/organization.schema.json`), which is TTL-shape-aligned.
