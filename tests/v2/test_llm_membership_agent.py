...
from git_metadata_extractor.agents.llm.membership import agent as membership_agent_module
...
def test_llm_membership_agent_records_strict_schema_warnings() -> None:
    ...
    # `_strict_validate` path emits at least one warning. (The agent
    # now silently drops malformed-but-typed dates rather than warning,
    # so exercise the warning path via extra-property rejection.)
