...
) -> tuple[InfoscienceRagProvider, _FakeStore, _FakeEmbedder, _FakeReranker | None]:
    store = _FakeStore()
    ...
    hits = [
        {
            ...
            "payload": {
                ...
                "doi": "10.1234/foo",
                "year": 2024,
                "infoscience_url": "https://infoscience.epfl.ch/record/1",
                "text": body_text,
...
def test_search_tool_factory_wires_provider() -> None:
    ...
    tool = make_infoscience_rag_search_tool(provider)

    assert tool.name == "search_infoscience_rag"
...
def test_fetch_chunks_tool_factory_wires_provider() -> None:
    ...
    tool = make_infoscience_rag_fetch_chunks_tool(provider)

    assert tool.name == "fetch_infoscience_chunks"
...
def test_fetch_records_tool_factory_returns_empty_on_chunks_collection() -> None:
    provider, _store, _emb, _rer = _make_provider(lookup_recs=[])
    tool = make_infoscience_rag_fetch_records_tool(provider)

    assert tool.name == "fetch_infoscience_records"
