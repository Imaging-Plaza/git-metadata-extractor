...
async def example_github_user_caching():
    ...
    print(f"   📊 User: {user_data.login if hasattr(user_data, 'login') else 'Unknown'}")
    ...
    print(f"   📊 User: {user_data_cached.login if hasattr(user_data_cached, 'login') else 'Unknown'}")
...
async def example_github_org_caching():
    ...
    second_call_time = time.time() - start_time
    print(f"   ⏱️  Time: {second_call_time:.3f}s")
    print(f"   📊 Org: {org_data_cached.login if hasattr(org_data_cached, 'login') else 'Unknown'}")
...
async def example_force_refresh():
    ...
    start_time = time.time()
    user_data_fresh = cache_manager.get_cached_or_fetch(
        api_type="github_user",
        params={"username": username},
        fetch_func=fetch_user_data,
        force_refresh=True
    ...
    print(f"   🔄 Fresh data fetched from GitHub API")
