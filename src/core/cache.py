...
class APICache:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.cache_db_path) as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_api_type ON cache_entries(api_type)")
    ...
    def get(self, api_type: str, params: Dict[str, Any], force_refresh: bool = False) -> Optional[Any]:
        """
    ...
    def set(self, api_type: str, params: Dict[str, Any], response_data: Any, ttl_days: Optional[int] = None) -> None:
        """
        ...
        logger.info(f"Cached {api_type} response with key {cache_key[:8]}... (expires in {ttl} days)")
    ...
    def invalidate(self, api_type: str, params: Dict[str, Any]) -> bool:
        ...
        with sqlite3.connect(self.cache_db_path) as conn:
            ...
            if cursor.rowcount > 0:
                logger.info(f"Invalidated cache entry for {api_type} with key {cache_key[:8]}...")
                return True
            else:
                logger.info(f"No cache entry found for {api_type} with key {cache_key[:8]}...")
                return False
