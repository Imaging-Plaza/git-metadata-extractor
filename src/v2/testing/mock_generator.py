"""Seed-based mock dataset generator for v2 extraction tests."""
...
DEFAULT_SEED = 42
DEFAULT_PERSON_COUNT = 5
DEFAULT_REPOSITORY_COUNT = 4
DEFAULT_ORGANIZATION_COUNT = 5
DEFAULT_GITHUB_ORGANIZATION_COUNT = 2
DEFAULT_ARTICLE_COUNT = 3

DATASET_KEY_TO_FILE = {
    "persons": "pulse_PersonShape.json",
    "repositories": "pulse_RepositoryShape.json",
    "organizations": "pulse_OrganizationShape.json",
    "memberships": "pulse_MembershipShape.json",
    "contributions": "pulse_ContributionShape.json",
    "articles": "pulse_ArticleShape.json",
}
...
_UUID_RANDOM: random.Random | None = None


def _set_uuid_random(rng: random.Random | None) -> None:
    global _UUID_RANDOM
    _UUID_RANDOM = rng
...
def make_uuid4() -> str:
    if _UUID_RANDOM is None:
        return str(uuid.uuid4())
    return _deterministic_uuid4(_UUID_RANDOM)
...
def generate_dataset(
    ...
    persons: int = DEFAULT_PERSON_COUNT,
    repos: int = DEFAULT_REPOSITORY_COUNT,
    orgs: int = DEFAULT_ORGANIZATION_COUNT,
    github_orgs: int = DEFAULT_GITHUB_ORGANIZATION_COUNT,
    articles: int = DEFAULT_ARTICLE_COUNT,
    edge_cases: bool = False,
) -> dict[str, list[dict[str, Any]]]:
    """Generate a deterministic, cross-reference-consistent mock dataset."""
    rng = random.Random(seed)
    _set_uuid_random(rng)

    try:
        organizations = generate_organizations(orgs, github_orgs, rng)
        people = generate_persons(persons, rng)
        repositories = generate_repositories(repos, people, organizations, rng)
        memberships = generate_memberships(people, organizations, rng)
        contributions = generate_contributions(people, repositories, rng)
        scholarly_articles = generate_articles(articles, people, organizations, rng)

        if edge_cases:
            edge_entities = generate_edge_cases(people, repositories, organizations)
            people.extend(edge_entities["persons"])
            repositories.extend(edge_entities["repos"])
            organizations.extend(edge_entities["orgs"])
            memberships.extend(edge_entities["memberships"])
            contributions.extend(edge_entities["contributions"])
            scholarly_articles.extend(edge_entities["articles"])
...
def write_dataset(
    ...
    """Write generated dataset files and return output paths by dataset key."""
    output_path = Path(output_dir)
    output_path.mkdir(parents=True, exist_ok=True)
    written_paths: dict[str, Path] = {}

    for dataset_key, file_name in DATASET_KEY_TO_FILE.items():
        entities = dataset.get(dataset_key)
        ...
        written_paths[dataset_key] = target_path
