...
from src.v2.agents.terminal.pi_config import PiProviderSpec, write_pi_config
from src.v2.agents.terminal.schema_cheatsheet import build_cheatsheet, get_shape_constraints
...
class TerminalRunner:
    ...
    def _invoke_agent(
        ...
        *,
        ...
        bash_ext = project_root / "src" / "v2" / "agents" / "terminal" / "pi_extension" / "index.ts"
        ext_paths: list[Path] = [bash_ext, *(self.extra_extensions or [])]
        skill_args: list[str] = []
        for skill_name in self.skills:
            skill_dir = project_root / "src" / "v2" / "skills" / skill_name
    ...
    def _validate(self, payload: dict[str, Any] | None) -> tuple[bool, list[str]]:
        ...
        try:
            ...
            from src.v2.validation import (  # noqa: PLC0415
...
def _load_jsonld_context() -> Any:
    ...
    `src/v2/schema/json/context/v2.0.jsonld`; we inline the mapping into
    ...
    """
    try:
        from src.v2.schema import load_jsonld_context  # noqa: PLC0415
