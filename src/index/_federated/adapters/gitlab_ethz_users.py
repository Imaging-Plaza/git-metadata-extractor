"""Adapter wrapping `src.index.gitlab_ethz_users` for federated search/lookup."""
...
class GitLabEthzUsersAdapter:
    ...
    def search(
        ...
        *,
        ...
        try:
            from src.index.gitlab_ethz_users.retrieval import search  # noqa: PLC0415
    ...
    def lookup(self, identifier: str) -> list[EntityRecord]:  # noqa: PLR0911
        ...
        if kind != "user" or host != "gitlab.ethz.ch":
            return []
        try:
            from src.index.gitlab_ethz_users.store import open_store  # noqa: PLC0415
        except Exception:  # noqa: BLE001
            return []
        store = None
        try:
