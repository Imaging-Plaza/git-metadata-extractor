...
from .core.cache_manager import get_cache_manager, extract_force_refresh_param
...
app = FastAPI(
    ...
    * **Cache Management**: Monitor and control the caching system
    ...
    contact={
        ...
        "email": "quentin.chappuis@epfl.ch"
    },
    license_info={
        "name": "MIT License",
...
def index():
    ...
    """
    return {"title": f"Hello, welcome to the Git Metadata Extractor v2.0.0. Gimie Version 0.7.2. LLM Model {os.environ['MODEL']}"}

@app.get("/v1/extract/json/{full_path:path}", tags=["Repository"])
async def extract(full_path: str, force_refresh: bool = Query(False, description="Force refresh from external APIs, bypassing cache")):
    """
    ...
    **Returns**: 
    ...
    def fetch_llm_data():
        return llm_request_repo_infos(str(full_path), output_format="json-ld", max_tokens=30000)
    ...
    try:
        # Get LLM data (cached)
        llm_result = cache_manager.get_cached_or_fetch(
            api_type="llm",
            params=cache_params,
...
async def extract_jsonld(full_path: str):
    ...
    **Note**: This endpoint does not use caching and always fetches fresh data.
...
async def get_org_json(full_path: str, force_refresh: bool = Query(False, description="Force refresh from external APIs, bypassing cache")):
    """
...
async def get_user_json(full_path: str, force_refresh: bool = Query(False, description="Force refresh from external APIs, bypassing cache")):
    """
...
async def gimie(full_path: str, force_refresh: bool = Query(False, description="Force refresh from external APIs, bypassing cache")):
    """
...
async def invalidate_cache(api_type: str, params: dict = None):
    ...
    **Returns**: 
    ...
    if success:
        ...
        return {"message": f"No cache entries found for {api_type} with given parameters"}
