...
def log_validation_errors(validation_error: ValidationError, context: str = ""):
    ...
    logger.error(f"Validation failed {context}: {len(validation_error.errors())} errors")
...
class SoftwareSourceCode(BaseModel):
    ...
    def validate_author_with_logging(cls, v):
        ...
        if isinstance(v, list):
            ...
            # Summary
            if missing_names:
                logger.warning(f"  🚨 {len(missing_names)} authors missing names: {', '.join(missing_names)}")
            else:
        ...
        if isinstance(v, list):
            ...
            for i, author in enumerate(v[:5]):  # Show first 5
                if isinstance(author, dict):
                    ...
                    total_commits = commits.get('total', 0) if isinstance(commits, dict) else 0
                    logger.debug(f"    [{i+1}] {name} ({email}) - {total_commits} commits")
    ...
    def validate_has_parameter_with_logging(cls, v):
        ...
        if isinstance(v, list):
                ...
                if isinstance(param, dict):
                    ...
                    if has_dim is not None:
                        if isinstance(has_dim, int) and has_dim > 0:
                            ...
                            logger.warning(f"  ⚠️ {name}: invalid dimensionality = {has_dim} (type: {type(has_dim)})")
                    else:
            ...
            # Summary
            if dim_issues:
                logger.warning(f"  🚨 {len(dim_issues)} parameters with dimensionality issues: {', '.join(dim_issues)}")
            else:
                logger.debug(f"  ✅ All {len(valid_params)} parameters have valid dimensionality")
        else:
    ...
    def validate_model_with_logging(self):
        ...
        logger.debug(f"  🔧 Git Authors: {len(self.gitAuthors) if self.gitAuthors else 0}")
        logger.debug(f"  ⚙️ Parameters: {len(self.hasParameter) if self.hasParameter else 0}")
        logger.debug(f"  🏷️ Repository Type: {getattr(self, 'repositoryType', 'Not set')}")
    ...
    def validate_related_to_organizations_with_logging(cls, v):
        ...
        if isinstance(v, list):
            ...
            for org in v:
                if org:
                    org_lower = org.lower().strip()  # Convert to lowercase for comparison
                    
                    if org_lower not in seen_lower:
                        unique_orgs.append(org)      # Keep original case
                        seen_lower.add(org_lower)    # Store lowercase version
                    else:
                        logger.debug(f"  🔄 Removed case-insensitive duplicate: '{org}' (matches existing)")
