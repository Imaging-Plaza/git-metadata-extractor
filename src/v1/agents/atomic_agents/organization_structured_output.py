"""
Organization structured output agent - Second stage of atomic agent pipeline.

This agent takes compiled context and simplified schema instructions
to produce structured metadata output for organizations.
"""
...
# Load model configurations for organization structured output
ORGANIZATION_STRUCTURED_OUTPUT_CONFIGS = load_model_config(
    "run_organization_structured_output",
)

# Validate configurations
for config in ORGANIZATION_STRUCTURED_OUTPUT_CONFIGS:
    if not validate_config(config):
        logger.error(
            f"Invalid configuration for organization structured output: {config}",
        )
        raise ValueError("Invalid model configuration")

# Generate simplified model dynamically from GitHubOrganization
...
# System prompt for organization structured output agent
ORGANIZATION_STRUCTURED_OUTPUT_SYSTEM_PROMPT = """
You are an expert at extracting structured metadata from organization information.
...
def get_organization_structured_output_prompt(
    compiled_context: CompiledContext,
    schema: Dict[str, Any],
    example: Optional[Dict[str, Any]] = None,
) -> str:
    """
    Generate prompt for organization structured output agent.
    ...
    """
    prompt = f"""Extract basic identity metadata from the compiled organization context.
...
async def generate_organization_structured_output(
    compiled_context: CompiledContext,
    schema: Dict[str, Any],
    example: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    """
    Generate structured output from compiled organization context.

    Args:
        compiled_context: Compiled context from organization context compiler
        schema: Simplified schema definition
        example: Optional example output
    ...
    # Prepare the prompt
    prompt = get_organization_structured_output_prompt(
        compiled_context,
        schema,
        example,
    )
    ...
    try:
        ...
        result = await run_agent_with_fallback(
            ORGANIZATION_STRUCTURED_OUTPUT_CONFIGS,
            prompt,
            agent_context,
            _SIMPLIFIED_MODEL,
            ORGANIZATION_STRUCTURED_OUTPUT_SYSTEM_PROMPT,
        ...
        estimated = estimate_tokens_with_tools(
            system_prompt=ORGANIZATION_STRUCTURED_OUTPUT_SYSTEM_PROMPT,
            user_prompt=prompt,
        ...
        logger.info(
            f"Organization structured output generated: {len(output_dict)} top-level fields",
        )
        ...
        logger.error(
            f"Organization structured output generation failed: {e}",
            exc_info=True,
        )
        return {
            "data": None,
