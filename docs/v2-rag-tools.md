...
LLM agents in the v2 pipeline can query six Qdrant-backed indices on
demand: Infoscience, HuggingFace Hub, OpenAlex, Zenodo, ORCID, and ROR.
Each index has a thin async **provider** in
...
| `HuggingFaceRagProvider` (`src/v2/ingest/providers/huggingface_rag.py`) | `search_huggingface_rag` | `models`, `datasets`, `spaces`, `orgs` | `library_name, pipeline_tag, sdk, gated, downloads, downloads_all_time, likes, author, license, entity_type` |
...
| `ZenodoRagProvider` (`src/v2/ingest/providers/zenodo_rag.py`) | `search_zenodo_rag` | `zenodo_records` | `year, doi, resource_type, access_right, entity_type, zenodo_id` |
...
Infoscience is the only index with chunked bodies — search hits return a
short snippet plus an `article_uuid` the agent can pass to
`fetch_infoscience_chunks` for the full text. The other indices already
return complete records from search.
...
| Agent | Tools to add |
|---|---|
| `article` | `openalex_rag` (works), `zenodo_rag` |
| `person` | `orcid_rag`, `openalex_rag` (authors) |
| `organization` | `ror_rag`, `huggingface_rag` (orgs), `openalex_rag` (institutions) |
| `repository` | `huggingface_rag` (models/datasets) |

Infoscience is already wired into article / person / organization agents.
...
src/v2/ingest/providers/
  ...
  huggingface_rag.py         # search (models/datasets/spaces/orgs)
  openalex_rag.py            # search (6 entity collections)
  zenodo_rag.py              # search (single collection)
...
src/v2/agents/llm/agent_tools/
  ...
  huggingface_rag.py         # 1 tool
  openalex_rag.py            # 1 tool
  zenodo_rag.py              # 1 tool
  orcid_rag.py               # 1 tool
  ror_rag.py                 # 1 tool

src/v2/dependencies.py       # _resolve_rag_provider + ProviderSet builder
src/v2/agents/models.py      # ProviderSet (6 *_rag fields)
```
