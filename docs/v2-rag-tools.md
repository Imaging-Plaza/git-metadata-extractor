...
[`federated-search.md`](https://github.com/caviri/open-pulse-sources/blob/main/docs/federated-search.md) for the full design.
...
| `ZenodoRagProvider` (`src/v2/ingest/providers/zenodo_rag.py`) | `search_zenodo_rag`, `fetch_zenodo_records` | `zenodo_records` | `year, doi, resource_type, access_right, entity_type, zenodo_id`. `fetch_zenodo_records(ids)` accepts both `zenodo_id` (canonical version-record) and `concept_recid` (parent / "all versions" id Zenodo redirects from), reading full bodies — title, doi, description, license, dates — from DuckDB rather than Qdrant. Use after `search_zenodo_rag` to hydrate hits, or to confirm a citation by id. See [`zenodo-index.md`](https://github.com/caviri/open-pulse-sources/blob/main/docs/zenodo-index.md). |
